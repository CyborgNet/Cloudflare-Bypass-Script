...
async function tryToSolve(sitekey, host, userAgent) {
  const headers = {
...
async function solveCaptcha(url, userAgent, options = {}) {
  ...
  while (true) {
    try {
      const result = await tryToSolve(siteKey, hostname, userAgent);
      if (result) {
        return result;
      }
    } catch (e) {
